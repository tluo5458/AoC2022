...
public class D24P2 {
	...
	public static int ans() {
		...
		State s = new State(goalX, goalY);
		s.t = ret1;
		added = new HashMap<Integer, HashSet<Integer>>();
		added.put(s.t, new HashSet<Integer>());
		q = new ArrayDeque<State>();
		q.add(s);
		added.get(s.t).add(toInt(goalX, goalY));
		int ret2 = -1;
		while (!q.isEmpty()) {
			State curr = q.pop();
		...
		s2.t = ret2;
		added = new HashMap<Integer, HashSet<Integer>>();
		added.put(s2.t, new HashSet<Integer>());
		q = new ArrayDeque<State>();
		q.add(s2);
		added.get(s2.t).add(1);
		while (!q.isEmpty()) {
			State curr = q.pop();
			if (curr.x == goalX && curr.y == goalY) {
				return curr.t;
