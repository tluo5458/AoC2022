...
public class D07P2 {
	private static class Node {
		...
		public int minAbove(int thresh) {
			int currBest = Integer.MAX_VALUE;
			if (children.size() == 0) {
				return currBest;
			}
			int curr = this.totalSize();
			if (currBest > curr && curr >= thresh) {
				currBest = curr;
			}
			for (Node n : children.values()) {
				int ncurr = n.minAbove(thresh);
				if (currBest > ncurr && ncurr >= thresh) {
					currBest = ncurr;
				}
			}
			return currBest;
	...
	public static int ans() {
		...
		return head.minAbove(head.totalSize() - 40000000);
	}
